...
import {EmailCheckProps, PasswordChangeProps, PasswordChangePropsWiwh} from "@/app/globalRedux/users/types";
import {fetchRecover_pass_change_password, fetchRecover_pass_CheckEmail} from "@/app/globalRedux/users/asyncActions";
import {useSearchParams} from "next/navigation";

export default function Pass_RecovForm({lang}: { lang: Locale }) {
 ...
 const onSubmit = handleSubmit((data) => {
  ...
  console.log(data)
     if(data.password === data.password2 && token !== null){

         const fulldata: PasswordChangePropsWiwh = {
             password: data.password,
             token, // use the token variable directly
         }
         dispatch(fetchRecover_pass_change_password(fulldata))
             .then((response) => {
         ...
         </button>
         {
             error && (
                 ...
                 <div id="toast-danger"
                     ...
                     <div className="ms-3 text-sm font-normal">{lang === "en" ? <>An error occurred, please try later!</> : <>Произошла ошибка, попробуйте позже!</>}</div>
                     <button type="button"
         ...
         }
         {
             ...
             errorNotEdenticalPass && (
                 <div id="toast-danger"
                     ...
                     <div className="ms-3 text-sm font-normal">{lang === "en" ? <>The passwords are not the
                         same.</> : <>Пароли не одинаковые.</>}</div>
                     <button type="button"
