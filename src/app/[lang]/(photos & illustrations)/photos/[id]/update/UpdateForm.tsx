...
import {fetchCreatePost, fetchOnePost, fetchUpdatePost} from "@/app/globalRedux/posts/asyncActions";
...
export default function UpdateForm(params: DetailPostInfoProps) {
            ...
            .then((response) => {
                ...
                if (response.meta.requestStatus === 'rejected') {
                    ...
                    if (response.payload) {
                        ...
                        // @ts-ignore
                        setValue('tags', response.payload.tags)

                        // if(response.payload.license === "Unsplash+"){
                        //     setValue('license_boolean',true)
                        //     // console.log(watch('license_boolean'))
                        // }else{
                        //     setValue('license_boolean',false)
                        //     // console.log(watch('license_boolean'))
                        // }
                        ...
                        // setValue('',response.payload.tags)
    ...
    const onSubmit = handleSubmit((values) => {
        ...
        if (data !== null) {
            ...
            if (typeof values.tags === 'string') {
                values.tags = values.tags.trim().replace(/\s*,\s*/g, ',').replace(/^\,|\,$/g, '').replace(/,+/g, ',');
                // @ts-ignore
                values.tags = values.tags.split(/\s*,\s*/).filter(Boolean);
                console.log(1)
            ...
            console.log(values)
            values._id = id;
            dispatch(fetchUpdatePost(values))
                .then((response) => {
                    console.log(response)
                    if (response.meta.requestStatus === 'rejected') {
                        setError(true);
        ...
        <form onSubmit={onSubmit}>
            ...
            <div className="flex flex-col items-start  justify-center w-full sm:px-4">
                            ...
                            <span className="font-medium">{lang === "en" ? <>Ensure that these requirements are
                                ...
                                <ul className="mt-1.5 list-disc list-inside flex flex-col items-start ">
                                    ...
                                    <li>{lang === "en" ? <>You can enter text as in the placeholder:</> : <>Можно
                                        вводить текст как в подсказке:</>} 1/300
                                    </li>
                ...
                <button type="submit"
                    ...
                    {lang === "en" ? <>Update post</> : <>Обновить запись</>}
                </button>
                {
                    error && (
                ...
                }
                {
                    SuccessDataChange && (
                        ...
                        <div id="toast-success"
                            ...
                            <div className="ms-3 text-sm font-normal">{lang === "en" ? <>Post updated.</> : <>Запись
                                обновлено.</>}</div>
                            <button type="button"
